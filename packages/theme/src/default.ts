import type { CloudSlug, ContainerKind, NodeKind } from '@bromaid/core';
import type {
  ContainerTheme,
  NodeTypeTheme,
  SlugIcon,
  SlugTheme,
  Surface,
  Theme,
} from './types.js';
import { generatedSlugIcons } from './generated-icons.js';

const ICON_24: Pick<SlugIcon, 'viewBox'> = { viewBox: '0 0 24 24' };

const iconGitHub: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12',
    },
  ],
};

const iconKeycloak: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'm18.742 1.182-12.493.002C4.155 4.784 2.079 8.393 0 12.002c2.071 3.612 4.162 7.214 6.252 10.816l12.49-.004 3.089-5.404h2.158v-.002H24L23.996 6.59h-2.168zM8.327 4.792h2.081l1.04 1.8-3.12 5.413 3.117 5.403-1.035 1.81H8.327a2047.566 2047.566 0 0 0-4.168-7.204C5.547 9.606 6.937 7.2 8.327 4.792Zm6.241 0 2.086.003c1.393 2.405 2.78 4.813 4.166 7.222l-4.167 7.2h-2.08c-.382-.562-1.038-1.808-1.038-1.808l3.123-5.405-3.124-5.413z',
    },
  ],
};

// Simple vendor glyphs for container title pills.
// These are intentionally minimal so they remain legible at 12px.
const iconAws: SlugIcon = {
  ...ICON_24,
  paths: [
    // three stacked chevrons / "a" impression
    { d: 'M6 16.8 12 6l6 10.8h-2.2L12 10.9 8.2 16.8H6Z' },
    { d: 'M8.1 18.6h7.8v1.6H8.1z' },
  ],
};

const iconAzure: SlugIcon = {
  ...ICON_24,
  paths: [
    // angular "A" style triangle mark
    { d: 'M12.2 3 4.6 20.8h4.3l1.6-4h6.2l2.7 4H24L14.6 3h-2.4Zm-.6 5.8 2.8 5.2H9.7l1.9-5.2Z' },
  ],
};

const iconGcp: SlugIcon = {
  ...ICON_24,
  paths: [
    // minimal cloud ring
    { d: 'M10.2 18.8h6.5c2.3 0 4.1-1.6 4.1-3.8 0-1.9-1.3-3.4-3.1-3.7C17.1 8.1 14.9 6 12.2 6 9.8 6 7.8 7.6 7 9.9 5.3 10.4 4 11.9 4 13.7c0 2.9 2.5 5.1 6.2 5.1Zm0-1.7c-2.9 0-4.5-1.6-4.5-3.4 0-1.2.8-2.2 2.1-2.6l.8-.2.2-.8c.5-1.7 2-2.7 3.7-2.7 2.1 0 3.8 1.6 3.9 3.8l.1.8.8.1c1.4.2 2.5 1.3 2.5 2.9 0 1.3-1.1 2.1-2.4 2.1h-6.7Z' },
  ],
};

const iconSlack: SlugIcon = {
  ...ICON_24,
  paths: [
    // Minimal hashtag-like mark (monochrome, legible at small sizes).
    { d: 'M9 3h2v18H9V3Zm4 0h2v18h-2V3ZM3 9h18v2H3V9Zm0 4h18v2H3v-2Z' },
  ],
};

const iconSharePoint: SlugIcon = {
  ...ICON_24,
  paths: [
    // Minimal "document" mark.
    { d: 'M7 3h7l3 3v15H7V3Zm7 1.8V7h2.2L14 4.8Z' },
    { d: 'M9 10h6v1.8H9V10Zm0 4h6v1.8H9V14Zm0 4h6v1.8H9V18Z' },
  ],
};

const iconCloudflare: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M16.5088 16.8447c.1475-.5068.0908-.9707-.1553-1.3154-.2246-.3164-.6045-.499-1.0615-.5205l-8.6592-.1123a.1559.1559 0 0 1-.1333-.0713c-.0283-.042-.0351-.0986-.021-.1553.0278-.084.1123-.1484.2036-.1562l8.7359-.1123c1.0351-.0489 2.1601-.8868 2.5537-1.9136l.499-1.3013c.0215-.0561.0293-.1128.0147-.168-.5625-2.5463-2.835-4.4453-5.5499-4.4453-2.5039 0-4.6284 1.6177-5.3876 3.8614-.4927-.3658-1.1187-.5625-1.794-.499-1.2026.119-2.1665 1.083-2.2861 2.2856-.0283.31-.0069.6128.0635.894C1.5683 13.171 0 14.7754 0 16.752c0 .1748.0142.3515.0352.5273.0141.083.0844.1475.1689.1475h15.9814c.0909 0 .1758-.0645.2032-.1553l.12-.4268zm2.7568-5.5634c-.0771 0-.1611 0-.2383.0112-.0566 0-.1054.0415-.127.0976l-.3378 1.1744c-.1475.5068-.0918.9707.1543 1.3164.2256.3164.6055.498 1.0625.5195l1.8437.1133c.0557 0 .1055.0263.1329.0703.0283.043.0351.1074.0214.1562-.0283.084-.1132.1485-.204.1553l-1.921.1123c-1.041.0488-2.1582.8867-2.5527 1.914l-.1406.3585c-.0283.0713.0215.1416.0986.1416h6.5977c.0771 0 .1474-.0489.169-.126.1122-.4082.1757-.837.1757-1.2803 0-2.6025-2.125-4.727-4.7344-4.727',
    },
  ],
};

const iconStripe: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.545-2.354 1.545-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 22.99 8.385 24 11.714 24c2.641 0 4.843-.624 6.328-1.813 1.664-1.305 2.525-3.236 2.525-5.732 0-4.128-2.524-5.851-6.594-7.305h.003z',
    },
  ],
};

const iconPostHog: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M9.854 14.5 5 9.647.854 5.5A.5.5 0 0 0 0 5.854V8.44a.5.5 0 0 0 .146.353L5 13.647l.147.146L9.854 18.5l.146.147v-.049c.065.03.134.049.207.049h2.586a.5.5 0 0 0 .353-.854L9.854 14.5zm0-5-4-4a.487.487 0 0 0-.409-.144.515.515 0 0 0-.356.21.493.493 0 0 0-.089.288V8.44a.5.5 0 0 0 .147.353l9 9a.5.5 0 0 0 .853-.354v-2.585a.5.5 0 0 0-.146-.354l-5-5zm1-4a.5.5 0 0 0-.854.354V8.44a.5.5 0 0 0 .147.353l4 4a.5.5 0 0 0 .853-.354V9.854a.5.5 0 0 0-.146-.354l-4-4zm12.647 11.515a3.863 3.863 0 0 1-2.232-1.1l-4.708-4.707a.5.5 0 0 0-.854.354v6.585a.5.5 0 0 0 .5.5H23.5a.5.5 0 0 0 .5-.5v-.6c0-.276-.225-.497-.499-.532zm-5.394.032a.8.8 0 1 1 0-1.6.8.8 0 0 1 0 1.6zM.854 15.5a.5.5 0 0 0-.854.354v2.293a.5.5 0 0 0 .5.5h2.293c.222 0 .39-.135.462-.309a.493.493 0 0 0-.109-.545L.854 15.501zM5 14.647.854 10.5a.5.5 0 0 0-.854.353v2.586a.5.5 0 0 0 .146.353L4.854 18.5l.146.147h2.793a.5.5 0 0 0 .353-.854L5 14.647z',
    },
  ],
};

const iconIntercom: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M21 0H3C1.343 0 0 1.343 0 3v18c0 1.658 1.343 3 3 3h18c1.658 0 3-1.342 3-3V3c0-1.657-1.342-3-3-3zm-5.801 4.399c0-.44.36-.8.802-.8.44 0 .8.36.8.8v10.688c0 .442-.36.801-.8.801-.443 0-.802-.359-.802-.801V4.399zM11.2 3.994c0-.44.357-.799.8-.799s.8.359.8.799v11.602c0 .44-.357.8-.8.8s-.8-.36-.8-.8V3.994zm-4 .405c0-.44.359-.8.799-.8.443 0 .802.36.802.8v10.688c0 .442-.36.801-.802.801-.44 0-.799-.359-.799-.801V4.399zM3.199 6c0-.442.36-.8.802-.8.44 0 .799.358.799.8v7.195c0 .441-.359.8-.799.8-.443 0-.802-.36-.802-.8V6zM20.52 18.202c-.123.105-3.086 2.593-8.52 2.593-5.433 0-8.397-2.486-8.521-2.593-.335-.288-.375-.792-.086-1.128.285-.334.79-.375 1.125-.09.047.041 2.693 2.211 7.481 2.211 4.848 0 7.456-2.186 7.479-2.207.334-.289.839-.25 1.128.086.289.336.25.84-.086 1.128zm.281-5.007c0 .441-.36.8-.801.8-.441 0-.801-.36-.801-.8V6c0-.442.361-.8.801-.8.441 0 .801.357.801.8v7.195z',
    },
  ],
};

const iconTailscale: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M24 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm-9 9a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm0-9a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm6-6a3 3 0 1 1 0-6 3 3 0 0 1 0 6zm0-.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5zM3 24a3 3 0 1 1 0-6 3 3 0 0 1 0 6zm0-.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5zm18 .5a3 3 0 1 1 0-6 3 3 0 0 1 0 6zm0-.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5zM6 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm9-9a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm-3 2.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5zM6 3a3 3 0 1 1-6 0 3 3 0 0 1 6 0zM3 5.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z',
    },
  ],
};

const iconCoolify: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M4.364 4.364V0h17.454v4.364zm0 13.09H0V4.365h4.364zm0 0h17.454v4.364H4.364ZM6.545 6.546v-1.7H22.3V2.182H24v4.363zm0 0v10.4h-1.7v-10.4ZM3.882 17.936v1.7h-1.7v-1.7ZM24 24H6.545v-1.7H22.3v-2.664H24Z',
    },
  ],
};

const iconNextjs: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M18.665 21.978C16.758 23.255 14.465 24 12 24 5.377 24 0 18.623 0 12S5.377 0 12 0s12 5.377 12 12c0 3.583-1.574 6.801-4.067 9.001L9.219 7.2H7.2v9.596h1.615V9.251l9.85 12.727Zm-3.332-8.533 1.6 2.061V7.2h-1.6v6.245Z',
    },
  ],
};

const iconRedis: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M22.71 13.145c-1.66 2.092-3.452 4.483-7.038 4.483-3.203 0-4.397-2.825-4.48-5.12.701 1.484 2.073 2.685 4.214 2.63 4.117-.133 6.94-3.852 6.94-7.239 0-4.05-3.022-6.972-8.268-6.972-3.752 0-8.4 1.428-11.455 3.685C2.59 6.937 3.885 9.958 4.35 9.626c2.648-1.904 4.748-3.13 6.784-3.744C8.12 9.244.886 17.05 0 18.425c.1 1.261 1.66 4.648 2.424 4.648.232 0 .431-.133.664-.365a100.49 100.49 0 0 0 5.54-6.765c.222 3.104 1.748 6.898 6.014 6.898 3.819 0 7.604-2.756 9.33-8.965.2-.764-.73-1.361-1.261-.73zm-4.349-5.013c0 1.959-1.926 2.922-3.685 2.922-.941 0-1.664-.247-2.235-.568 1.051-1.592 2.092-3.225 3.21-4.973 1.972.334 2.71 1.43 2.71 2.619z',
    },
  ],
};

const iconDatadog: SlugIcon = {
  ...ICON_24,
  paths: [
    {
      d: 'M19.57 17.04l-1.997-1.316-1.665 2.782-1.937-.567-1.706 2.604.087.82 9.274-1.71-.538-5.794zm-8.649-2.498l1.488-.204c.241.108.409.15.697.223.45.117.97.23 1.741-.16.18-.088.553-.43.704-.625l6.096-1.106.622 7.527-10.444 1.882zm11.325-2.712l-.602.115L20.488 0 .789 2.285l2.427 19.693 2.306-.334c-.184-.263-.471-.581-.96-.989-.68-.564-.44-1.522-.039-2.127.53-1.022 3.26-2.322 3.106-3.956-.056-.594-.15-1.368-.702-1.898-.02.22.017.432.017.432s-.227-.289-.34-.683c-.112-.15-.2-.199-.319-.4-.085.233-.073.503-.073.503s-.186-.437-.216-.807c-.11.166-.137.48-.137.48s-.241-.69-.186-1.062c-.11-.323-.436-.965-.343-2.424.6.421 1.924.321 2.44-.439.171-.251.288-.939-.086-2.293-.24-.868-.835-2.16-1.066-2.651l-.028.02c.122.395.374 1.223.47 1.625.293 1.218.372 1.642.234 2.204-.116.488-.397.808-1.107 1.165-.71.358-1.653-.514-1.713-.562-.69-.55-1.224-1.447-1.284-1.883-.062-.477.275-.763.445-1.153-.243.07-.514.192-.514.192s.323-.334.722-.624c.165-.109.262-.178.436-.323a9.762 9.762 0 0 0-.456.003s.42-.227.855-.392c-.318-.014-.623-.003-.623-.003s.937-.419 1.678-.727c.509-.208 1.006-.147 1.286.257.367.53.752.817 1.569.996.501-.223.653-.337 1.284-.509.554-.61.99-.688.99-.688s-.216.198-.274.51c.314-.249.66-.455.66-.455s-.134.164-.259.426l.03.043c.366-.22.797-.394.797-.394s-.123.156-.268.358c.277-.002.838.012 1.056.037 1.285.028 1.552-1.374 2.045-1.55.618-.22.894-.353 1.947.68.903.888 1.609 2.477 1.259 2.833-.294.295-.874-.115-1.516-.916a3.466 3.466 0 0 1-.716-1.562 1.533 1.533 0 0 0-.497-.85s.23.51.23.96c0 .246.03 1.165.424 1.68-.039.076-.057.374-.1.43-.458-.554-1.443-.95-1.604-1.067.544.445 1.793 1.468 2.273 2.449.453.927.186 1.777.416 1.997.065.063.976 1.197 1.15 1.767.306.994.019 2.038-.381 2.685l-1.117.174c-.163-.045-.273-.068-.42-.153.08-.143.241-.5.243-.572l-.063-.111c-.348.492-.93.97-1.414 1.245-.633.359-1.363.304-1.838.156-1.348-.415-2.623-1.327-2.93-1.566 0 0-.01.191.048.234.34.383 1.119 1.077 1.872 1.56l-1.605.177.759 5.908c-.337.048-.39.071-.757.124-.325-1.147-.946-1.895-1.624-2.332-.599-.384-1.424-.47-2.214-.314l-.05.059a2.851 2.851 0 0 1 1.863.444c.654.413 1.181 1.481 1.375 2.124.248.822.42 1.7-.248 2.632-.476.662-1.864 1.028-2.986.237.3.481.705.876 1.25.95.809.11 1.577-.03 2.106-.574.452-.464.69-1.434.628-2.456l.714-.104.258 1.834 11.827-1.424zM15.05 6.848c-.034.075-.085.125-.007.37l.004.014.013.032.032.073c.14.287.295.558.552.696.067-.011.136-.019.207-.023.242-.01.395.028.492.08.009-.048.01-.119.005-.222-.018-.364.072-.982-.626-1.308-.264-.122-.634-.084-.757.068a.302.302 0 0 1 .058.013c.186.066.06.13.027.207m1.958 3.392c-.092-.05-.52-.03-.821.005-.574.068-1.193.267-1.328.372-.247.191-.135.523.047.66.511.382.96.638 1.432.575.29-.038.546-.497.728-.914.124-.288.124-.598-.058-.698m-5.077-2.942c.162-.154-.805-.355-1.556.156-.554.378-.571 1.187-.041 1.646.053.046.096.078.137.104a4.77 4.77 0 0 1 1.396-.412c.113-.125.243-.345.21-.745-.044-.542-.455-.456-.146-.749',
    },
  ],
};

const slugs = new Map<CloudSlug | string, SlugTheme>([
  ['aws', { color: '#FF9900', label: 'AWS', icon: iconAws }],
  ['aws:s3', { color: '#7AA116', label: 'S3' }],
  ['aws:lambda', { color: '#FF9900', label: 'λ' }],
  ['aws:rds', { color: '#3B48CC', label: 'RDS' }],
  ['aws:guardduty', { color: '#FF9900', label: 'GD' }],
  ['aws:inspector', { color: '#FF9900', label: 'IN' }],
  ['aws:cloudtrail', { color: '#FF9900', label: 'CT' }],
  ['aws:eventbridge', { color: '#FF9900', label: 'EB' }],
  ['aws:ses', { color: '#FF9900', label: 'SES' }],
  ['entra', { color: '#0078D4', label: 'EID' }],
  ['ssm', { color: '#FF9900', label: 'SSM' }],
  ['traefik', { color: '#24A1C1', label: 'TR' }],
  ['oauth2-proxy', { color: '#7f8c8d', label: 'O2' }],
  ['hatchet', { color: '#f39c12', label: 'H' }],
  ['kuma', { color: '#e67e22', label: 'KU' }],
  ['azure', { color: '#0078D4', label: 'AZ', icon: iconAzure }],
  ['azure:openai', { color: '#10A37F', label: 'AI' }],
  ['gcp', { color: '#4285F4', label: 'GCP', icon: generatedSlugIcons.gcp }],
  ['gcp:gemini', { color: '#8E75B2', label: 'AI' }],
  ['github', { color: '#888888', label: 'GH', icon: generatedSlugIcons.github }],
  ['cloudflare', { color: '#F38020', label: 'CF', icon: generatedSlugIcons.cloudflare }],
  ['stripe', { color: '#635BFF', label: '$', icon: generatedSlugIcons.stripe }],
  ['keycloak', { color: '#4D4D4D', label: 'KC', icon: generatedSlugIcons.keycloak }],
  ['posthog', { color: '#F54E00', label: 'PH', icon: generatedSlugIcons.posthog }],
  ['intercom', { color: '#6AFDEF', label: 'IC', icon: generatedSlugIcons.intercom }],
  ['tailscale', { color: '#000000', label: 'TS', icon: generatedSlugIcons.tailscale }],
  ['coolify', { color: '#00D9FF', label: 'CO', icon: generatedSlugIcons.coolify }],
  ['nextjs', { color: '#000000', label: 'N', icon: generatedSlugIcons.nextjs }],
  ['redis', { color: '#DC382D', label: 'R', icon: generatedSlugIcons.redis }],
  ['datadog', { color: '#632CA6', label: 'DD', icon: generatedSlugIcons.datadog }],
  ['slack', { color: '#4A154B', label: 'SL', icon: iconSlack }],
  ['sharepoint', { color: '#0078D4', label: 'SP', icon: iconSharePoint }],
]);

const containers = new Map<ContainerKind, ContainerTheme>([
  [
    'region',
    { fill: 'rgba(120,120,120,0.05)', stroke: 'rgba(160,160,160,0.55)', dashed: false },
  ],
  [
    'vpc',
    { fill: 'rgba(74,144,226,0.05)', stroke: 'rgba(74,144,226,0.6)', dashed: false },
  ],
  [
    'subnet.public',
    { fill: 'rgba(242,166,39,0.06)', stroke: 'rgba(242,166,39,0.7)', dashed: true },
  ],
  [
    'subnet.private',
    { fill: 'rgba(232,90,90,0.06)', stroke: 'rgba(232,90,90,0.7)', dashed: true },
  ],
  [
    'group',
    { fill: 'rgba(99,153,34,0.06)', stroke: 'rgba(99,153,34,0.65)', dashed: false },
  ],
]);

const nodeTypes = new Map<NodeKind, NodeTypeTheme>([
  ['db', { color: '#9b59b6' }],
  ['actor', { color: '#5dade2' }],
  ['external', { color: '#95a5a6' }],
  ['s3', { color: '#7AA116' }],
  ['service', { color: '#16a085' }],
  ['addon', { color: '#7f8c8d' }],
  ['node', { color: '#888888' }],
]);

const lightSurface: Surface = {
  bg: '#fafafa',
  bg2: '#ffffff',
  bg3: '#f0f0f0',
  fg: '#1a1a1a',
  fgDim: '#555555',
  fgFaint: '#999999',
  border: '#dddddd',
  borderStrong: '#bbbbbb',
};

const darkSurface: Surface = {
  bg: '#0f0f10',
  bg2: '#18181a',
  bg3: '#1f1f22',
  fg: '#e8e8e8',
  fgDim: '#9a9a9a',
  fgFaint: '#6a6a6a',
  border: '#2a2a2e',
  borderStrong: '#3a3a3f',
};

export const defaultTheme: Theme = {
  slugs,
  containers,
  nodeTypes,
  surface: { light: lightSurface, dark: darkSurface },
};
